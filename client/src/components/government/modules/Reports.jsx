...
const Reports = () => {
  ...
  const [reportRegion, setReportRegion] = useState('National');
  ...
  const fallbackReports = [
    { id: 'r1', title: 'National Situation Report — Weekly', type: 'Situation Report', status: 'Final', date: '27 Jul 2026', pages: 24, author: 'NDMA HQ' },
    { id: 'r2', title: 'Cyclone Preparedness Assessment', type: 'Assessment', status: 'Draft', date: '26 Jul 2026', pages: 18, author: 'IMD' },
    { id: 'r3', title: 'District-wise Resource Audit', type: 'Audit', status: 'Final', date: '25 Jul 2026', pages: 42, author: 'Resource Cell' },
    { id: 'r4', title: 'Flood Response After-Action Review', type: 'AAR', status: 'Final', date: '24 Jul 2026', pages: 36, author: 'SDRF' },
    { id: 'r5', title: 'AI Model Performance Report — Q2', type: 'Analytics', status: 'Draft', date: '23 Jul 2026', pages: 12, author: 'AI Lab' },
    { id: 'r6', title: 'Inter-Agency Coordination Summary', type: 'Coordination', status: 'Final', date: '22 Jul 2026', pages: 8, author: 'Command Centre' },
  ];
          ...
          {recentReports.map((r, i) => (
            <div key={i} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button onClick={() => showToast(`Downloading ${r.title} as PDF...`, 'info')} className="text-[9px] font-semibold text-indigo-600 hover:text-indigo-800 px-2 py-1 rounded hover:bg-indigo-50">
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {['Situation Report', 'Mission Summary', 'Resource Audit', 'After-Action', 'Analytics'].map((t) => (
                  <button key={t} onClick={() => setReportType(t)} className={`text-[9px] font-semibold px-2 py-1 rounded transition-all ${t === reportType ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{t}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {['National', 'Karnataka', 'Mangaluru', 'District'].map((r) => (
                  <button key={r} onClick={() => setReportRegion(r)} className={`text-[9px] font-semibold px-2 py-1 rounded transition-all ${r === reportRegion ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{r}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-1">
                {['PDF', 'DOCX', 'XLSX', 'PPTX'].map((f) => (
                  <button key={f} onClick={() => setReportFormat(f)} className={`text-[9px] font-semibold px-3 py-1.5 rounded transition-all ${f === reportFormat ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{f}</button>
                ))}
              </div>
            </div>
            <button onClick={async () => {
              ...
              try {
                const res = await apiFetch('/v2/government/reports', {
                  method: 'POST',
                  body: JSON.stringify({ title: `${reportType} — ${reportRegion}`, type: reportType, scope: reportRegion, format: reportFormat }),
                });
                if (res.ok) {
                  showToast(`${reportType} for ${reportRegion} generated as ${reportFormat}`, 'success');
                } else {
                  showToast('Report queued for generation', 'info');
                }
              } catch (err) {
            ...
            }} disabled={generating} className="w-full px-4 py-2 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-50">
              <i className="fas fa-file-export mr-1" /> {generating ? 'Generating...' : 'Generate Report'}
            </button>
          </div>
        </div>
        <div className="rounded-xl bg-white border border-slate-200 p-4 shadow-sm">
