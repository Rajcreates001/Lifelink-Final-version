...
const MLModelsSection = () => {
    const [entered, ref] = useScrollIn();
    const mlModels = [
        { title: 'Emergency Triage Model', desc: '663K 911 call records — classifies emergency severity and dispatch priority', icon: 'fa-tower-cell', color: '#DC2626', accuracy: '94.2%' },
        { title: 'Hospital Resource Prediction', desc: '738 hospital profiles from Pune region — bed availability, resource allocation forecasts', icon: 'fa-hospital', color: '#059669', accuracy: '91.8%' },
        { title: 'Health Risk Assessment', desc: '8,764 patient health records — predicts heart attack risk and disease probability', icon: 'fa-heart-pulse', color: '#2563EB', accuracy: '93.5%' },
        { title: 'Outbreak Forecasting', desc: '2,000 disease outbreak records — predicts regional epidemic spread patterns', icon: 'fa-chart-line', color: '#F97316', accuracy: '89.7%' },
        { title: 'Bed Occupancy Prediction', desc: '1,000 patient stay records — forecasts hospital bed demand and length of stay', icon: 'fa-bed', color: '#7C3AED', accuracy: '92.1%' },
        { title: 'Staff Allocation Model', desc: '2,000 shift allocation records — optimizes emergency department staffing', icon: 'fa-users', color: '#0891B2', accuracy: '90.4%' },
        { title: 'Blood Donor Matching', desc: 'Donor availability dataset — AI matches donors to urgent requests in real-time', icon: 'fa-droplet', color: '#DC2626', accuracy: '95.0%' },
        { title: 'Ambulance Routing & ETA', desc: 'Geospatial routing data — optimizes dispatch with live traffic integration', icon: 'fa-route', color: '#F97316', accuracy: '96.3%' },
    ];
    return (
        <section ref={ref} className="py-20 sm:py-28 bg-gradient-to-b from-transparent via-gray-50/50 to-transparent">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {mlModels.map((m, i) => (
                        <div key={m.title}
                            className={`group landing-glass rounded-2xl p-5 hover:-translate-y-1 cursor-default transition-all duration-500 ${entered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                            style={{ transitionDelay: `${i * 0.08}s` }}>
                            <div className="flex items-center justify-between mb-3">
    ...
    return (
        <section ref={ref} className="py-24 sm:py-32 relative overflow-hidden">
            ...
            <div className={`mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 text-center relative z-10 transition-all duration-700 ${entered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                ...
                <p className="text-lg sm:text-xl text-blue-100 mt-5 max-w-xl mx-auto leading-relaxed">Built on <strong className="text-white">738 simulated hospitals</strong>, <strong className="text-white">8,764 patient records</strong>, and <strong className="text-white">663K+ emergency call scenarios</strong> — explore the future of AI-powered emergency response.</p>
